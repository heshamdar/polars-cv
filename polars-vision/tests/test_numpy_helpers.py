...
functions that parse the polars-vision numpy sink output format.
"""
...
from polars_vision import numpy_dtype, numpy_from_bytes, numpy_header_size, numpy_shape
...
def create_test_bytes(dtype_code: int, shape: list[int], data: bytes) -> bytes:
    """
    Create test bytes in the polars-vision numpy format.
