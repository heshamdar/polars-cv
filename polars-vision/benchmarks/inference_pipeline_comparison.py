"""
Batch Inference Pipeline Comparison: HuggingFace/torchvision vs polars-vision.
...
2. polars-vision batch preprocessing pipeline
...
For training workloads with random augmentation, polars-vision handles
...
# 3. Preprocess data BEFORE creating the DataLoader (what polars-vision does)
...
def benchmark_huggingface_pipeline(
    ...
    preprocessing, which is the fair comparison to polars-vision's approach.
    ...
    # This is the fair comparison to polars-vision's batch preprocessing
...
def benchmark_polars_vision_pipeline(
    metadata_path: Path,
    batch_size: int = 32,
    num_workers: int = 4,
    num_epochs: int = 1,
    ...
    """
    Benchmark polars-vision batch preprocessing pipeline.

    This uses polars-vision for batch preprocessing with the file_path source,
    ...
    from polars_vision import IMAGENET_MEAN, IMAGENET_STD, Pipeline, numpy_from_bytes

    mode = "streaming" if use_streaming else "eager"
    log(f"Setting up polars-vision pipeline (mode={mode})...")
    ...
    log("Preprocessing all images with polars-vision...")
    preprocess_start = time.perf_counter()
...
def verify_output_equivalence(
    ...
    algorithms between torchvision (PIL-based) and polars-vision (Rust-based).
    ...
    log(f"  Loading polars-vision metadata from {metadata_path}...")
    ...
    if max_diff < tolerance:
        ...
        log("   (torchvision uses PIL, polars-vision uses Rust/Lanczos)", "WARN")
        return False
...
def run_inference_serving(
    ...
    # --- polars-vision Inference ---
    log_step("INFERENCE: polars-vision")
    ...
    log("Loading and preprocessing with polars-vision...")
    preprocess_start = time.perf_counter()
    df = pl.read_parquet(metadata_path)
    ...
    results["polars_vision"] = total_samples / pv_time
    log_timing("Total inference time", pv_time)
    log(f"  Final throughput: {results['polars_vision']:.1f} samples/sec")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Compare HuggingFace and polars-vision for batch inference preprocessing"
    )
    parser.add_argument(
    ...
    print()
    print("=" * 95)
    print("BATCH INFERENCE PIPELINE COMPARISON: HuggingFace vs polars-vision")
    print("=" * 95)
    print()
    print(
    ...
    try:
        ...
        log_step("BENCHMARK: polars-vision (eager)")
        pv_result = benchmark_polars_vision_pipeline(
        ...
        log_step("BENCHMARK: polars-vision (streaming)")
        pv_streaming_result = benchmark_polars_vision_pipeline(
            dataset.metadata_path,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            num_epochs=args.epochs,
        ...
        if not args.skip_serving:
            ...
            pv_tp = serving_results["polars_vision"]
            print(f"  HuggingFace:    {hf_tp:.1f} samples/sec")
            print(f"  polars-vision:  {pv_tp:.1f} samples/sec")
            if hf_tp > 0:
                speedup = pv_tp / hf_tp
