...
fn test_blob_roundtrip() {
    ...
    // Use internal reshape for test setup logic
    // (In real usage we'd use ViewExpr, but here we want raw buffer manipulation)
    // Since reshape is internal, we simulate strided view via slicing if possible, 
    // or rely on the fact that to_blob calls to_contiguous.
    // Let's rely on ViewBuffer's layout.
    
    // We can't access reshape directly here as it is pub(crate). 
    // We will use the public API via ViewExpr to create a reshaped result.
    let buf = view_buffer::ViewExpr::new_source(buf)
        .reshape(vec![4, 2])
        .plan()
        .execute();
