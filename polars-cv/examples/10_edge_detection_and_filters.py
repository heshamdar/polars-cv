...
def demo_convolution() -> None:
    ...
    emboss = (
        Pipeline().source("image_bytes").grayscale().convolve2d(kernel=emboss_kernel)
    )
    edge = Pipeline().source("image_bytes").grayscale().convolve2d(kernel=edge_kernel)
    blur = (
        Pipeline()
        .source("image_bytes")
        .grayscale()
        .convolve2d(kernel=blur_kernel, normalize=True)
    )
