...
| File | Responsibility |
|------|---------------|
| `lib.rs` | PyO3 module entry, `vb_graph` expression function, `unified_output_dtype`, and the planner-facing FFI (`plan_step`, `node_pass`, `enum_catalog`, `op_catalog`, `io_catalog`, `plan_source`, `plan_sink`) |
