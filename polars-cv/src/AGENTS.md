...
| File | Responsibility |
|------|---------------|
| `lib.rs` | PyO3 module entry, `vb_graph` expression function, `unified_output_dtype`, and the planner-facing FFI (`plan_step`, `op_contract`, `enum_catalog`, `op_catalog`, `io_catalog`, `io_check`) |
