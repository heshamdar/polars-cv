...
| `params.rs` | `ParamValue` — literal vs expression parameter resolution for the ops still on the legacy wire (`resolve_*` numerics, `resolve_str` for non-structural enums, `req_*_literal` for structural ones); typed ops read `Param<T>` instead. `ParamCtx::probe` marks the plan-time shape probe, where every expression param is bound to an integer placeholder and the enum/flag accessors fall back to their default; real execution stays strict |
