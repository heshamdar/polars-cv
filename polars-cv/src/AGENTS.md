...
- `CompiledGraph::operand` distinguishes "node is in the graph but produced no output for this row" (→ null this node too) from "node is not in the graph" (→ error). Every cross-node operand read (`Binary`, `ApplyMask`, `ChannelMerge`, rasterize shape refs) goes through it, so a null upstream propagates instead of raising "references unknown node".
