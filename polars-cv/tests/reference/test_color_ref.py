...
class TestRgbBgr:
    ...
    def test_rgb_to_bgr(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe = (
            Pipeline()
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="bgr")
        )
    ...
    def test_bgr_to_rgb_roundtrip(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe = (
            ...
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="bgr")
            .convert_color(from_space="bgr", to_space="rgb")
        )
        actual = _run_pipe(pipe, rgb_png)
...
class TestRgbHsv:
    ...
    def test_hsv_roundtrip(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe = (
            ...
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="hsv")
            .convert_color(from_space="hsv", to_space="rgb")
        )
        actual = _run_pipe(pipe, rgb_png)
...
class TestRgbYcbcr:
    ...
    def test_ycbcr_roundtrip(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe = (
            Pipeline()
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="ycbcr")
            .convert_color(from_space="ycbcr", to_space="rgb")
        )
        actual = _run_pipe(pipe, rgb_png)
...
class TestRgbLab:
    ...
    def test_lab_roundtrip(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe = (
            Pipeline()
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="lab")
...
class TestEdgeCases:
    ...
    def test_noop_conversion(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe = (
            Pipeline()
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="rgb")
        )
        actual = _run_pipe(pipe, rgb_png)
        np.testing.assert_array_equal(actual, rgb_image)
    ...
    def test_invalid_color_space(self) -> None:
        ...
        with pytest.raises(ValueError, match="unknown ColorSpace"):
            Pipeline().source("image_bytes").convert_color(
                from_space="rgb", to_space="xyz"
            )
...
class TestCrossConversions:
    ...
    def test_bgr_to_hsv(self, rgb_image: np.ndarray, rgb_png: bytes) -> None:
        ...
        pipe_direct = (
            ...
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="bgr")
            .convert_color(from_space="bgr", to_space="hsv")
        )
        pipe_via_rgb = (
            Pipeline()
            .source("image_bytes")
    ...
    def test_hsv_to_ycbcr(self, rgb_png: bytes) -> None:
        ...
        pipe = (
            ...
            .source("image_bytes")
            .convert_color(from_space="rgb", to_space="hsv")
            .convert_color(from_space="hsv", to_space="ycbcr")
            .convert_color(from_space="ycbcr", to_space="rgb")
        )
