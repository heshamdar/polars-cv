...
> | C7 — Python surface fully generated | **done** — C7a: `convolve2d(ksize=)` dropped, the side is the kernel's. C7b: `source()` generated from `io_catalog.json` (the `Source` family doc is its docstring; `SOURCE_FIELDS` + `Pipeline._with_source`); `decode_max_size` is `Literal<NonZeroU32>`; `_validate_enum`/`_reject_expr` deleted. C7c: the lazy forwarders are generated methods (`_lazy_forwarders.py`, from the built `Pipeline`); `_install_pipeline_forwarders`, `_make_forwarder`, `lazy.pyi`, `gen_lazy_stub.py` and the `ty` exclusion deleted (three untruthful annotations it hid, fixed). C7d: each op's `Domain:` line generated from its domain contract (`domains` in the op catalogue, structural choices varied). Deviations (user decisions): `source()` has no canvas keywords (write `.rasterize()`); no stub at all rather than a stub written by `gen_ops.py` |
> | C8 — Sweep, guards and docs | **done** — absence scans whose subject is gone deleted (`test_histogram_schema_declared_once`: the method is generated; `test_no_duplicate_expected_dtype_enum`: subsumed by the dtype-table scan); removed-symbol gate complete through C8; the per-row rule restated (no effect on rank, dtype or domain; a per-row size is unknown at plan time) in `CLAUDE.md`, `view_buffer::mode` and the user guide; module and canonical-path tables updated. Also, by user decision: a fixed-size `Array` column's shape is planned; `sobel`/`laplacian` lost `ksize`; sugar `Domain:` lines are composed from declared ops. The line-count ledger is dropped (user: not a goal) |
