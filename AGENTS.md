...
| Which ops exist | Rust `ops::TypedOp` (`typed_ops!` registry; `LEGACY_OPS` is empty until P6 deletes it) ↔ Python `OP_NAMES` | `typed_and_legacy_ops_partition_the_op_set` (disjoint, and together the frozen op set), `test_op_names_matches_rust_known_ops_without_the_plugin` (works with no `.so`); a name in neither set fails deserialization |
...
| Which formats a `source()` / `.sink()` parameter applies to | Sinks: the typed per-format structs in `polars-cv/src/formats/sink.rs` (`formats!` registry → `io_catalog.json` → generated `SinkFormat`), checked from `.sink()` over `io_check`. Sources: `SOURCE_PARAM_APPLIES` in `_types.py`, read by `reject_inapplicable_params` (typed in the next P4 step) | Deserialization: a field the format does not read is refused naming where it applies; `test_param_applicability.py` sweeps parameter × format grids from the catalogue and checks the `quality` claim against the encoders; the source table's keys must equal `source()`'s keywords and the check must read `locals()` |
...
cannot refuse an unknown key — is gone: every op is a closed struct (its
`#[derive(Op)]` refuses to compile without `deny_unknown_fields`). The
`LegacyOpSpec` variant survives only until P6 deletes it; nothing on the wire
reaches it (`LEGACY_OPS` is empty), and `resolve_op` refuses one built by hand.
