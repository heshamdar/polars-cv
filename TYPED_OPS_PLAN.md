...
- **P9 — `Op::shape() -> OpShape`, not `infer_dims(&[&[Dim]])`.** Symbolic
  input sizes are not enough: a per-row parameter must be symbolic too, and a
  resolved engine op holds only concrete values. So each op returns its shape
  transform as data (`OpShape`, one evaluator for both known and symbolic
  sizes), and each typed op builds the same `OpShape` from its own fields
  (`OpDef::shape`, required) — two constructors, one arithmetic, held together
  by `typed_shape_is_the_resolved_steps`.
- **P9 — `ParamCtx::probe` survives as `ParamCtx::planning`.** Shapes no
  longer need it, but the planner still resolves each op once, without a row,
  to read its rules (domain, dtype, rank, channels, identity); a per-row value
  cannot change those, so one placeholder (`WireScalar::planning_value`) is
  sound. Removing it would mean typing every rule at the typed-op layer too.
  The four probe values, the variation detection and the enum-default
