...
**polars-cv/src/**
- `lib.rs` — PyO3 module entry, `vb_graph` polars expression function, dtype inference, and the `plan_step`/`node_pass`/`enum_catalog`/`op_catalog`/`io_catalog`/`io_check` FFI the Python planner reads (`plan.rs` holds `plan_step`, one call per appended op; `passes.rs` the node-scope optimisation passes)
