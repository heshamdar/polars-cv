...
| File | Role |
|------|------|
| `pipeline.py` | `Pipeline` — inherits the generated ops and `source()`; hand-written sugar declares the ops it lowers to (`_sugar`) |
| `lazy.py` | `LazyPipelineExpr` — `.pipe()`, `.merge_pipe()`, `.sink()`, the multi-operand ops; inherits the generated chainable methods |
| `_ops_generated.py` | Generated by `scripts/gen_ops.py` from the catalogues: the enums, every op method and `source()` (`_OpsMixin`), the geometry accessor mixins, `OP_FIELDS`/`SOURCE_FIELDS`/`OP_DOMAINS` |
| `_lazy_forwarders.py` | Generated by `scripts/gen_ops.py` from the built `Pipeline`: `LazyPipelineExpr`'s chainable methods (`_LazyForwardersMixin`) |
| `_domains.py` | The one `Domain:` docstring renderer, for generated methods and `_sugar` (dependency-free, so the generator loads it by path) |
...
- `formats/` — the typed sources and sinks: the `Source`/`Sink` families, one `#[derive(Ops)]` variant per format
- `execute.rs` — source decoding helpers (image bytes) and byte-sink encoding
...
| `contour` | Geometry vectors | `extract_contours()`, `source("contour")` |
...
3. Re-bless the catalogue (`POLARS_CV_BLESS=1 scripts/with-pyo3-env.sh cargo
   ...
   ones (`flip_h`, `sobel`, …), each declaring the ops it lowers to with
   `@_sugar(...)` (its `Domain:` line is composed from them, and
   `test_sugar_appends_exactly_the_ops_it_declares` holds it to them), and
   the `lazy_only` ops on `LazyPipelineExpr`.
