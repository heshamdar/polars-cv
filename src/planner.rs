...
use crate::expr::{ViewExpr, ExprNode};
    ...
    match op.kind {
        ImageOpKind::Threshold(thresh) => {
            if let Ok(view) = buf.as_image_view::<Luma<u8>>() {
                ...
                ViewBuffer::from_vec(new_data).reshape(vec![view.height as usize, view.width as usize, 1])
            } else {
        ...
        ImageOpKind::Resize { width, height, filter } => {
            ...
            if c == 3 {
                ...
                ViewBuffer::from_vec(resized.into_raw()).reshape(vec![height as usize, width as usize, 3])
            } else if c == 1 {
                let img_buf: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::from_raw(w, h, raw_vec)
                    .expect("Failed to create ImageBuffer from compatible buffer");
                let resized = imageops::resize(&img_buf, width, height, img_filter);
                ViewBuffer::from_vec(resized.into_raw()).reshape(vec![height as usize, width as usize, 1])
            } else {
