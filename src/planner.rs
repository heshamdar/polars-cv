...
use crate::expr::{TensorExpr, ExprNode};
use crate::ops::{ViewOp, ComputeOp, ImageOp, MemoryEffect, Op, ImageOpKind, FilterType};
    ...
    match op.kind {
        ImageOpKind::Threshold(thresh) => {
            ...
            if let Ok(view) = buf.as_image_view::<Luma<u8>>() {
                ...
                TensorBuffer::from_vec(new_data).reshape(vec![view.height as usize, view.width as usize, 1])
            } else {
                // Decision: Materialize if not compatible or if view creation failed
                let work_buf = if buf.layout.is_contiguous() { buf } else { buf.to_contiguous() };
                
                if work_buf.dtype() != DType::U8 {
                    panic!("Threshold op requires U8 dtype");
                }
                let count = work_buf.layout.num_elements();
                let src_slice = unsafe { std::slice::from_raw_parts(work_buf.as_ptr::<u8>(), count) };
                
                let new_data: Vec<u8> = src_slice.iter()
        ...
        ImageOpKind::Resize { width, height, filter } => {
            ...
            let work_buf = if buf.is_compatible_with(ExternalLayout::ImageCrate) && buf.layout.is_contiguous() {
            ...
            let (h, w, c) = (shape[0] as u32, shape[1] as u32, *shape.get(2).unwrap_or(&1) as u32);
            ...
            let count = work_buf.layout.num_elements();
            let raw_vec = unsafe { std::slice::from_raw_parts(work_buf.as_ptr::<u8>(), count).to_vec() };
            ...
            if c == 3 {
                ...
                TensorBuffer::from_vec(resized.into_raw()).reshape(vec![height as usize, width as usize, 3])
            } else if c == 1 {
                let img_buf: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::from_raw(w, h, raw_vec)
                    .expect("Failed to create ImageBuffer from compatible buffer");
                let resized = imageops::resize(&img_buf, width, height, img_filter);
                TensorBuffer::from_vec(resized.into_raw()).reshape(vec![height as usize, width as usize, 1])
            } else {
        ...
        ImageOpKind::Blur { sigma } => {
            ...
            if c == 3 {
                let img_buf: ImageBuffer<Rgb<u8>, Vec<u8>> = ImageBuffer::from_raw(w, h, raw_vec).unwrap();
                let blurred = imageops::blur(&img_buf, sigma);
                TensorBuffer::from_vec(blurred.into_raw()).reshape(vec![h as usize, w as usize, 3])
            } else {
                 let img_buf: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::from_raw(w, h, raw_vec).unwrap();
                let blurred = imageops::blur(&img_buf, sigma);
