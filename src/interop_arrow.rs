...
impl FromArrow for TensorBuffer {
    fn from_arrow_array(array: &dyn Array) -> Result<TensorBuffer, BufferError> {
        ...
        while let DataType::FixedSizeList(_field, size) = current_array.data_type() {
            ...
            let list_array = current_array.as_any()
                .downcast_ref::<FixedSizeListArray>()
                .ok_or(BufferError::TypeMismatch { expected: DType::U8, got: DType::U8 })?; 
        ...
        let (buffer, dtype) = match current_array.data_type() {
            DataType::Float32 => {
                let a = current_array.as_any().downcast_ref::<PrimitiveArray<Float32Type>>().unwrap();
                (a.values().inner().clone(), DType::F32)
            },
            DataType::Float64 => {
                let a = current_array.as_any().downcast_ref::<PrimitiveArray<Float64Type>>().unwrap();
                (a.values().inner().clone(), DType::F64)
            },
            DataType::Int32 => {
                let a = current_array.as_any().downcast_ref::<PrimitiveArray<Int32Type>>().unwrap();
                (a.values().inner().clone(), DType::I32)
            },
            DataType::Int64 => {
                let a = current_array.as_any().downcast_ref::<PrimitiveArray<Int64Type>>().unwrap();
                (a.values().inner().clone(), DType::I64)
            },
            DataType::UInt8 => {
                let a = current_array.as_any().downcast_ref::<PrimitiveArray<UInt8Type>>().unwrap();
                (a.values().inner().clone(), DType::U8)
            },
            DataType::FixedSizeBinary(size) => {
                let a = current_array.as_any().downcast_ref::<FixedSizeBinaryArray>().unwrap();
                shape.push(*size as usize);
                (a.to_data().buffers()[0].clone(), DType::U8)
            },
            DataType::Binary => {
                let a = current_array.as_any().downcast_ref::<BinaryArray>().unwrap();
                let buf = a.to_data().buffers()[1].clone();
                shape = vec![buf.len()]; 
                (buf, DType::U8)
            },
            DataType::LargeBinary => {
                let a = current_array.as_any().downcast_ref::<LargeBinaryArray>().unwrap();
                let buf = a.to_data().buffers()[1].clone();
                shape = vec![buf.len()];
                (buf, DType::U8)
            },
            _ => return Err(BufferError::TypeMismatch { 
                expected: DType::F32, 
                got: DType::U8
            }),
        };
...
impl ToArrow for TensorBuffer {
    fn to_arrow_binary(&self) -> BinaryArray {
        ...
        // 2. Get Raw Slice
        let slice = unsafe {
             std::slice::from_raw_parts(contig.as_ptr::<u8>(), total_bytes)
        };
        ...
        let offsets = arrow::buffer::OffsetBuffer::<i32>::from_lengths(std::iter::once(total_bytes));
        ...
        match contig.dtype() {
            DType::F32 => {
                ...
                for &dim_size in contig.shape().iter().skip(1).rev() {
                    let field = Arc::new(arrow::datatypes::Field::new("item", current_array.data_type().clone(), false));
